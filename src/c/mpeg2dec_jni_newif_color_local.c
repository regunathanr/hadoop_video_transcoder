...
void SaveFrame(uint8_t* pFrame, int width, int height, int iFrame) {
  ...
  // Open file
    ...
    pFile1=fopen(szFilename, "wb");
    if(pFile1==NULL)
        return;
    
    printf("inside save frameU %d \t %d\n",width/2,height);
    
    // Write header
    fprintf(pFile, "P6\n%d %d\n255\n", width/2,height);

    printf("inside save frameV %d \t %d\n",width/2,height);
    ...
    for(y = 0; y < height; y++)
    {
	   ...
	   // {
		...
		{
                	//writing U
            		fwrite(&(pFrame[offset+x + (y*widthby2)]),1,1,pFile);//r val
            		fwrite(&(pFrame[offset+x + (y*widthby2)]),1,1,pFile);//g val
            		fwrite(&(pFrame[offset+x + (y*widthby2)]),1,1,pFile);//b val
		}
		for(x = 0; x < width/4; x++)
		{
			//writing U
	  ...
	  // {
		for(x = 3*width/4; x < width; x++)
                {
			//writing V
                	fwrite(&(pFrame[offset+x + (y*widthby2)]),1,1,pFile1);//r val
                	fwrite(&(pFrame[offset+x + (y*widthby2)]),1,1,pFile1);//g val
                	fwrite(&(pFrame[offset+x + (y*widthby2)]),1,1,pFile1);//b val
		}
		for(x = width/2; x < 3*width/4; x++)
		{
			//writing V
        ...
        switch (state) {
            ...
            case STATE_INVALID_END:
                ...
                if(myframecnt > 3)
                {
                    ...
                    write_jpeg_file(info->display_fbuf->buf,info->sequence->width,info->sequence->height,myframecnt,0);
                }
                myframecnt++;

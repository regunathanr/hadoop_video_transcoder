...
static int num_frames_toskip=0;
...
void Rearrange_decframe(unsigned char** tempsrc,unsigned char* dest,int width,int height);
  ...
  w = Coded_Picture_Width>>1;
    ...
    for (j=0; j<Coded_Picture_Height; j++)
    {
      for (i=0; i<w; i++)
      {
        ...
        //printf("i: %d\n",i);
        //temp_val = (int)(21*(src[im2]+src[ip3])
          //              -52*(src[im1]+src[ip2])
            //           +159*(src[i]+src[ip1])+128)>>8;
        //printf("temp_val %d:\n",temp_val);
        temp_val = (0.5)*(src[im1]+src[ip1]);
        if(temp_val < 0)
        {
            temp_val = 0;
        }
...
void convert_yuv422p_rgb(unsigned char *src,unsigned char *pPPM,int offset, int incr, int height,int Coded_Picture_Width, int Coded_Picture_Height)
{
  ...
  int horizontal_size = Coded_Picture_Width;
   ...
   //     Error("malloc failed");
    u444 = (unsigned char*)calloc(sizeof(unsigned char),Coded_Picture_Width*Coded_Picture_Height);

  // if (!(v444 = (unsigned char *)malloc(Coded_Picture_Width*Coded_Picture_Height)))
    //    Error("malloc failed");

   v444 = (unsigned char*)calloc(sizeof(unsigned char),Coded_Picture_Width*Coded_Picture_Height);

   if((u444 == 0)||(v444 == 0))
   {
        printf("could not allocate memeory for u444\n");
        exit(1);
   }
   ...
   for(y = 0; y < height; y++)
    {
        for(x=0; x<width; x++)
        {
            dest[idx] = src[x+(y*width)];
            idx++;
        }
    }

    printf("idx after saving Y: %d\n",idx);
    src = tempsrc[1];
    offset = 0;
    widthby2 = width/2;
    for(y = 0; y < height; y++)
    {
        ...
        {
                dest[idx] = src[offset+x + (y*widthby2)];
                idx++;
        }
   }
...
void write_jpeg_file(uint8_t** pFrame, int w, int h,  int iFrame,int write_to_mem )
{
    ...
    yuv422buf = (unsigned char*)calloc(sizeof(unsigned char),w*h*2);
    ...
    for(y=0; y<image_height; y++)
    {
        ...
        {
            rgb_buf[idx] = pFrame[0][x + (y*image_width)];
            idx++;
            rgb_buf[idx] = pFrame[0][x + (y*image_width)];
            idx++;
            rgb_buf[idx] = pFrame[0][x + (y*image_width)];
    ...
    Rearrange_decframe(pFrame,yuv422buf,w,h);
    
    if(yuv422buf == 0)
    {
       printf("yuv422buf no allocated\n");
    }

    convert_yuv422p_rgb(yuv422buf,rgb_buf,0, w, h,w, h);
    ...
    cinfo.in_color_space = JCS_YCbCr;//JCS_RGB;//JCS_YCbCr; /* colorspace of input image */
    ...
    if(write_to_mem == 1)
    {
		...
		memcpy(jpg_outputbufferPtr,jpgbuff,numBytes_compjpg);
                ...
                jpg_outputbufferPtr = jpg_outputbufferPtr + numBytes_compjpg;
...
void write_data_ascii(char* mpegdec_config_fname, mpeg2dec_t* data )
{
   ...
   /* fprintf(fp,"%d\n",data->sequence.width);//width
    ...
    seq_info_c_array[0] = data->sequence.width;//width
    seq_info_c_array[1] = data->sequence.height;//height
    seq_info_c_array[2] = data->sequence.chroma_width;//chroma_width
    seq_info_c_array[3] = data->sequence.chroma_height;//chroma_height
    seq_info_c_array[4] = data->sequence.byte_rate; //byte_rate
    seq_info_c_array[5] = data->sequence.vbv_buffer_size; //vbv_buffer_size
    seq_info_c_array[6] = data->sequence.flags; //flags
    seq_info_c_array[7] = data->sequence.picture_width;//picture_width
    seq_info_c_array[8] = data->sequence.picture_height;//picture_height
    seq_info_c_array[9] = data->sequence.display_width;//display_width
    seq_info_c_array[10] = data->sequence.display_height;//display_height
    seq_info_c_array[11] = data->sequence.pixel_width;//pixel_width
    seq_info_c_array[12] = data->sequence.pixel_height;//pixel_height
    seq_info_c_array[13] = data->sequence.frame_period;//frame_period
    seq_info_c_array[14] = data->sequence.profile_level_id;//profile_level_id uint8_t
    seq_info_c_array[15] = data->sequence.colour_primaries;//colour_primaries uint8_t
    seq_info_c_array[16] = data->sequence.transfer_characteristics;//transfer_characteristics uint8_t
    seq_info_c_array[17] = data->sequence.matrix_coefficients;//matrix_coefficients uint8_t
        ...
        switch (state) {
            ...
            case STATE_SEQUENCE:
                ...
                if(mpeg2dec_state_ready == 0)
                {
                    ...
                    mpeg2dec_state_ready = 3;//implies that the parser shud exit after writing sequence info
                }
                break;
            case STATE_PICTURE:
                /* might skip */
            ...
            case STATE_INVALID_END:
                ...
                if(myframecnt > 3)
                {
                    if(num_frames_toskip != 0)
                    { 
                        if(myframecnt%num_frames_toskip == 0)
			{
                    		...
                    		write_jpeg_file(info->display_fbuf->buf,info->sequence->width,info->sequence->height,myframecnt,1);
			}
                    }
		    else
		    { 
...
static void es_loop_chunk(void)
{
    ...
    do {
        ...
        if(1)
        {
            ...
            if(mpeg2dec_state_ready == 3)
	    {
		printf("just writing out the header\n");
		break;
	    }
...
Java_VideoHeaderMapper_mpeg2decHeaderMain (JNIEnv *env, jobject obj, jbyteArray buffer_java, jint bufsize, jintArray sequence_info_arr)
{
    int i;
    int argc;
    char** argv;
    ...
    mpeg2dec_state_ready = 0;//if file mpeg2decstate_fname is already present

    bufferPtr = (*env)->GetByteArrayElements(env,buffer_java, NULL);
    buf_length = bufsize;    
    bufferPtr_copy = bufferPtr;
   ...
   // fclose (in_file);
    ...
    (*env)->ReleaseByteArrayElements(env, buffer_java, bufferPtr_copy, 0);
			...
			
{
    ...
    jpg_outputbufferPtr_copy = jpg_outputbufferPtr;
    ...
    num_frames_toskip = num_frames_toskip_java;
   ...
   // fclose (in_file);
    ...
    (*env)->ReleaseByteArrayElements(env, buffer_java, bufferPtr_copy, 0);
    printf("after releasing java input buffer\n");
   (*env)->ReleaseByteArrayElements(env, jpg_outputbuffer_java,jpg_outputbufferPtr_copy, 0);

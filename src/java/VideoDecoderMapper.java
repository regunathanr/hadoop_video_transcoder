...
public class VideoDecoderMapper extends Mapper<LongWritable,BytesWritable,Text, BytesWritable>
{
  ...
  int temp_byte_array_filled = 0;
   ...
   static {
	...
	// JNI load the var value 
      ...
      seq_filename = hdfs_dir + ((FileSplit)context.getInputSplit()).getPath().getName()+"_sequence_info.txt";
       ...
       //{
         ...
         // {
		   ...
		   System.arraycopy(frameid_bytes,0,temp_buf,0,8);
                   ...
                   String filename_val = ((FileSplit)context.getInputSplit()).getPath().getName()+"_"+chunk_val+"_"+actual_framenum+"_map.jpg"; 
                   String reducer_key = ((FileSplit)context.getInputSplit()).getPath().getName()+"_"+chunk_val;                  
                   //Path outFile = new Path("frame"+key.get()+"_"+i+".jpg");
                   //if(i > actual_num_frames_written_to_buf[0]-12)
                   int fr_num = 783+26;
                   String tempstring = reducer_key + "_" + fr_num + ".jpg"; 
		   if(filename_val.equals("SZTRA101a02.m2v_1_1134_map.jpg"))//(i == 0) //(i >= 0)&&(i < 26))
		   {	
